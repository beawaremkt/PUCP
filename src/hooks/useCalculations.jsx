...
export const useCalculations = () => {
  ...
  return useMemo(() => {
    ...
    const totalCostoHoras = state.horas.reduce((sum, h) => sum + ((h.horas || 0) * (h.tarifa || 0)), 0) || 0;

    // Total licencias
    const totalCostoLicencias = state.licencias.reduce((sum, l) => sum + (l.costo || 0), 0) || 0;
    ...
    const costoAntes = state.esfuerzoOperativo
      ...
      .reduce((sum, e) => sum + (e.costo || ((e.horas || 0) * (e.tarifa || 0))), 0) || 0;
    ...
    const volumenOperaciones = parseFloat(state.acta.lineaBaseCuantitativa.transacciones) || 0;
    ...
    const ahorroFinancieroAnual = (diferenciaCostos * volumenOperaciones * factorConversion) || 0;

    // ROI = (Ahorro Anual - Inversión) / Inversión * 100
    const roi = inversionTotal > 0 ? (((ahorroFinancieroAnual - inversionTotal) / inversionTotal) * 100) || 0 : 0;

    // Horas antes y después
    const horasAntes = state.esfuerzoOperativo
      .filter(e => e.momento === 'Antes de la implementación')
      .reduce((sum, e) => sum + (e.horas || 0), 0) || 0;
    ...
    const horasLiberadasAlAno = (diferenciaHoras * volumenOperaciones * factorConversion) || 0;
